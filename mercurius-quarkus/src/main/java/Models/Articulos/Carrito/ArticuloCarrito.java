...
public class ArticuloCarrito {
    ...
    public BigDecimal getTotalArticulo() {
        if (this.isPromo) {
            ...
            var precioArticulo = this.getArticulo().getLastPrecio().getPrecioConUtilidad();
    ...
    public BigDecimal getTotalArticulos() {
        var articulo = this;
        var precioArticulo = articulo.getArticulo().getLastPrecio().getPrecioConUtilidad();
        BigDecimal total;
        ...
        if (this.isPromo) {
            ...
            BigDecimal tax = BigDecimal.valueOf(articulo.getArticulo().getCodigoCabys().getImpuesto());
            ...
            total = precioFinal.multiply(getCantidad());
        } else {
            // For non-promo items, calculate total based on quantity directly
            total = precioArticulo.multiply(getCantidad());
        }
    ...
    public BigDecimal getArticuloConDescuento() {
        ...
        var precioConUtilidad = Articulo.getArticulo().getLastPrecio().getPrecioConUtilidad();
        double tax = Articulo.getArticulo().getCodigoCabys().getImpuesto();
    ...
    /**
     * Calculates the total tax for all unique tax rates present in the cart
     * items.
     *
    ...
    public static Map<Integer, BigDecimal> calculateTotalTaxForUniqueRates(List<ArticuloCarrito> carrito) {
        ...
        if (carrito != null && !carrito.isEmpty()) {
            for (ArticuloCarrito item : carrito) {
                int taxRate = item.getArticulo().getCodigoCabys().getImpuesto(); // Get tax rate from the item

                // Calculate the tax for the current item
                BigDecimal itemTaxTotal = item.getTotalImpuesto().multiply(item.cantidad);
