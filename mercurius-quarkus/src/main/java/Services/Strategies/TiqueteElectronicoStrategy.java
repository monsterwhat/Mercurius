...
import Models.Documentos.TiqueteElectronicoDocumento;
...
public class TiqueteElectronicoStrategy implements DocumentoStrategy {
    ...
    @Inject
    public TiqueteElectronicoStrategy(EmisorService emisorService, ReceptorService receptorService) {
    ...
    public Encabezado buildEncabezado(AppSettings appSettings, Clients selectedClient) {
        ...
        try {
            ...
            if (selectedClient != null && selectedClient.getName() != null) {
                Receptor receptor = buildReceptor(selectedClient);
    ...
    static Receptor buildReceptor(Clients selectedClient) {
        ...
        if (!"nacional".equals(selectedClient.getIdType().toLowerCase())) {
            receptor.setIdentificacionExtranjero(String.valueOf(selectedClient.getIdNumber()));
        } else {
            IdentificacionReceptor id = new IdentificacionReceptor();
            id.setNumero(String.valueOf(selectedClient.getIdNumber()));
