...
public class ClientsResource {
    ...
    /**
     * Cédula lookup via the free Gometa API (nombre + tipoIdentificacion)
     * to prefill the client form. Never 500s: unknown cédula or offline
     * service returns {@code ok=false} so the form stays usable.
     */
    @GET
    @Path("/lookup")
    @Produces(MediaType.APPLICATION_JSON)
    ...
    public Response lookupCedula(@QueryParam("cedula") @Nullable String cedula) {
        ...
        if (digits.length() < 9 || digits.length() > 12) {
            ...
            data.put("ok", false);
            data.put("mensaje", "Cédula inválida (9-12 dígitos)");
        ...
        try {
            ...
            HttpRequest req = HttpRequest.newBuilder(
                    URI.create("https://apis.gometa.org/cedulas/" + digits))
                    .timeout(Duration.ofSeconds(12)).GET().build();
            String body = http.send(req, HttpResponse.BodyHandlers.ofString()).body();
            JsonNode root = new ObjectMapper().readTree(body);
            if (root.path("resultcount").asInt(0) < 1) {
                Map<String, Object> data = new LinkedHashMap<>();
                data.put("ok", false);
                data.put("mensaje", "Cédula no encontrada");
                return Response.ok(ApiResponse.ok(data)).build();
            }
            Map<String, Object> data = new LinkedHashMap<>();
            data.put("ok", true);
            data.put("nombre", root.path("nombre").asText(""));
            data.put("tipoIdentificacion", root.path("tipoIdentificacion").asText(""));
            data.put("contribuyente", root.path("situacion").path("estado").asText("").toLowerCase(java.util.Locale.ROOT).contains("inscrito"));
            List<String> actividades = new ArrayList<>();
            for (JsonNode act : root.path("actividades")) {
                String codigo = act.path("codigo").asText("").trim();
                if (!codigo.isEmpty() && "A".equalsIgnoreCase(act.path("estado").asText(""))) {
                    actividades.add(codigo);
                }
            }
            data.put("actividades", actividades);
            return Response.ok(ApiResponse.ok(data)).build();
        } catch (Exception e) {
            LOG.warn("Cédula lookup failed | source=ClientsResource.lookupCedula()", e);
            Map<String, Object> data = new LinkedHashMap<>();
            data.put("ok", false);
            data.put("mensaje", "Servicio no disponible");
            return Response.ok(ApiResponse.ok(data)).build();
        }
    }
