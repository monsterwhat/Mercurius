...
/**
 * Reporte de ventas por familia for the NEW app surface — port of
 * {@code secured/pages/Articulos/Reportes/Familia/index.xhtml}
 * (plan task T19, read-only).
 *
 * <p>Backed DIRECTLY by {@link InventarioService#getTotalSalesByFamilia(Date,
 * Date)} — the same aggregate the legacy
 * {@code ReportesDiariosController.cargarVentasFamilias()} rendered. The legacy
 * controller accepted a user id it never used in the query; the port keeps only
 * the parameters that affect the result ({@code desde}/{@code hasta}).
 * The footer total uses the same
 * {@link ReportesFamiliasYDepartamentos#totalReportes(List)} helper.</p>
 */
@Path("/app/reportes/articulos/familias")
@Produces(MediaType.TEXT_HTML)
@RolesAllowed({"admin", "inventario"})
public class FamiliasResource {

    private static final String BASE_URL = "/app/reportes/articulos/familias";
    ...
    @Inject
    @Location("pages/reportes/familias")
    Template pagina;

    @Inject
    @Location("pages/reportes/_tablas/familias")
    Template tabla;
    ...
    public Response get(
        ...
        if (consultaCompleta) {
            List<ReportesFamiliasYDepartamentos> reportes =
                    inventarioService.getTotalSalesByFamilia(inicio, fin);
            if (reportes != null) {
        ...
        model.put("titulo", "Reporte de Ventas por Familia");
    ...
    private static List<Map<String, Object>> columnas() {
        return List.of(
                Map.of("label", "Familia", "key", "nombre"),
